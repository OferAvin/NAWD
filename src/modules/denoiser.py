...
class Denoiser():
    def __init__(self, train_dataset, noisy_dataset, model_adjustments, mode):
        self.model = None
    ...
    def denoise(self):
        n_days_labels = self.train_dataset.n_days_labels
        n_task_labels = self.train_dataset.n_task_labels
        
        signal_data_loader = DataLoader(dataset=self.train_dataset, batch_size=params['btch_sz'], shuffle=True, num_workers=0)
        self.model = convolution_AE(self.train_dataset.n_channels, n_days_labels, n_task_labels, self.model_adjustments, \
        ...
        # get data
        noisy_signal, _ = self.noisy_dataset.getAllItems()
